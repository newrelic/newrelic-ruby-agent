...
# Ruby <= 2.4.0 requires MySQL <= v5.5 (which uses OpenSSL <= v1.0)
# Source these from the home directory if available
def bundler_build_mysql2
  return unless Dir.exist?(docker_mysql_dir)

  "BUNDLE_BUILD__MYSQL2='--with-mysql-config=#{docker_mysql_dir}/bin/mysql_config'"
end
