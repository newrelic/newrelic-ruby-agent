  ...
  module Agent
    module DistributedTracing
      class TraceContextCrossAgentTest < Minitest::Test
        ...
        def accept_headers(test_case, _txn)
        ...
        def create_newrelic_payloads(test_case, _txn, w3c_payloads)
          outbound_payloads = []
          if test_case['outbound_newrelic_payloads']
            w3c_payloads.each do |w3c_payload|
              outbound_payloads << if newrelic_header = w3c_payload['newrelic']
                                     DistributedTracePayload.from_http_safe(newrelic_header)
                                   end
            end
          end
          outbound_payloads
        ...
        def create_payloads(test_case, txn)
          ...
          [1, payloads.count].max.times do |_index|
            outbound_headers = {}
        ...
        def intrinsics_for_event(test_case, event_type)
          unless ALLOWED_EVENT_TYPES.include? event_type
            raise %(Test fixture refers to unexpected event type "#{event_type}")
          end
        ...
        def verify_attributes(test_case_attributes, actual_attributes, event_type)
          ...
          (test_case_attributes['exact'] || []).each do |k, v|
            assert_equal v,
                         ...
                         %(Wrong "#{k}" #{event_type} attribute; expected #{v.inspect}, was #{actual_attributes[k.to_s].inspect})
          end
        ...
        def verify_transaction_intrinsics(test_case)
          ...
          actual_intrinsics, = last_transaction_event
        ...
        def verify_error_intrinsics(test_case)
          ...
          actual_intrinsics, = last_error_event
        ...
        def verify_metrics(test_case)
          expected_metrics = test_case['expected_metrics'].each_with_object({}) do |(metric_name, call_count), memo|
        ...
        def object_to_hash(object)
          object.instance_variables.each_with_object({}) do |variable_name, hash|
        ...
        def rack_format(_test_case, carrier)
          carrier ||= {}
          rack_headers = {}
