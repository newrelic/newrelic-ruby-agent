  ...
  module Agent
    class MessagingTest < Minitest::Test
      ...
      def test_agent_attributes_assigned_for_generic_wrap_consume_transaction
        ...
        assert transaction_event.is_a?(Array), "expected Array, actual: #{transaction_event.class}"
        assert_equal 3, transaction_event.length, "expected Array of 3 elements, actual: #{transaction_event.length}"
        assert transaction_event.all? { |e|
                 e.is_a?(Hash)
               }, "expected Array of 3 hashes, actual: [#{transaction_event.map(&:class).join(',')}]"
        assert transaction_event[2].key?(:'message.routingKey'),
               "expected transaction event attributes to have key :'message.routingKey', actual: #{transaction_event[2].keys.join(',')}"
        assert_equal 'red', transaction_event[2][:'message.routingKey']

        span_event = last_span_event
        assert span_event[2].key?(:'message.routingKey'),
               "expected span event attributes to have key :'message.routingKey', actual: #{span_event[2].keys.join(',')}"
      ...
      def test_header_attributes_assigned_for_generic_wrap_consume_transaction
        with_config "attributes.include": 'message.headers.*' do
          ...
          assert_equal 'foo', event[2][:"message.headers.token"],
                       "Expected header attributes to be added, actual attributes: #{event[2]}"
        end
      end
      ...
      def test_cat_headers_removed_when_headers_assigned_as_attributes
        with_config "attributes.include": 'message.headers.*' do
          ...
          refute event[2].has_key?(:"message.headers.NewRelicID"),
                 'Expected CAT headers to be omitted from message attributes'
        end
      end
      ...
      def test_consume_api_passes_message_properties_headers_to_underlying_api
        ...
        assert segment.is_a?(NewRelic::Agent::Transaction::MessageBrokerSegment)
      ...
      def test_consume_segments_filter_out_CAT_headers_from_parameters
        segment = NewRelic::Agent::Messaging.start_amqp_consume_segment(
          ...
          destination_name: 'Default',
          delivery_info: { routing_key: 'foo', exchange_name: 'bar' },
          message_properties: { headers: { 'hi' => 'there', 'NewRelicID' => '123#456',
                                           'NewRelicTransaction' => 'abcdef' } }
        )
        refute segment.params[:headers].key?('NewRelicID'),
               "expected segment params to not have CAT header 'NewRelicID'"
        refute segment.params[:headers].key?('NewRelicTransaction'),
               "expected segment params to not have CAT header 'NewRelicTransaction'"
      ...
      def test_consume_segments_filter_out_synthetics_headers_from_parameters
        ...
        refute segment.params[:headers].key?('NewRelicSynthetics'),
               'expected segment params to not have Synthetics header'
        assert segment.params[:headers].key?('hi'), 'expected segment params to have application defined headers'
        assert_equal 'there', segment.params[:headers]['hi']
      end
      ...
      def test_consume_segments_do_not_attach_empty_after_filtering_headers
        segment = NewRelic::Agent::Messaging.start_amqp_consume_segment(
          ...
          message_properties: { headers: { 'NewRelicID' => '123#456', 'NewRelicTransaction' => 'abcdef',
                                           'NewRelicSynthetics' => 'qwerasdfzxcv' } }
        )
        refute segment.params[:headers], 'expected no :headers key in segment params'
      end
      ...
      def test_agent_attributes_assigned_for_amqp_wrap_consume_transaction
        ...
        with_config "attributes.include": ['message.headers.*', 'message.replyTo', 'message.correlationId',
                                           'message.exchangeType'] do
          tap = mock 'tap'
          tap.expects :tap
          ...
          NewRelic::Agent::Messaging.wrap_amqp_consume_transaction(
            ...
            message_properties: { reply_to: 'reply.key', correlation_id: 'correlate',
                                  headers: { 'foo' => 'bar', 'NewRelicID' => 'baz' } },
      ...
      def test_segment_records_proper_metrics_for_consume
        in_transaction 'test_txn' do |_txn|
      ...
      def test_wrap_message_broker_consume_transaction_reads_cat_headers
        ...
        with_config "cross_application_tracer.enabled": true,
          ...
          NewRelic::Agent::Messaging.wrap_message_broker_consume_transaction(
            ...
            assert_equal txn.attributes.intrinsic_attributes_for(NewRelic::Agent::AttributeFilter::DST_TRANSACTION_TRACER),
