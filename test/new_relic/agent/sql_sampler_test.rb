...
class NewRelic::Agent::SqlSamplerTest < Minitest::Test
  ...
  def test_save_slow_sql
    ...
    data.sql_data.concat [
      ...
      NewRelic::Agent::SlowSql.new(NewRelic::Agent::Database::Statement.new('select * from test2'),
  ...
  def test_sql_aggregation
    ...
    slow_sql = NewRelic::Agent::SlowSql.new(NewRelic::Agent::Database::Statement.new(query), 'Database/test/select',
                                            1.2)

    sql_trace = NewRelic::Agent::SqlTrace.new(query, slow_sql, 'tx_name', 'uri')

    sql_trace.aggregate NewRelic::Agent::SlowSql.new(NewRelic::Agent::Database::Statement.new(query), 'Database/test/select', 1.5),
                        'slowest_tx_name', 'slow_uri'
    sql_trace.aggregate NewRelic::Agent::SlowSql.new(NewRelic::Agent::Database::Statement.new(query), 'Database/test/select', 1.1),
                        'other_tx_name', 'uri2'
  ...
  def test_harvest_should_aggregate_similar_queries
    ...
    queries = [
      NewRelic::Agent::SlowSql.new(
        NewRelic::Agent::Database::Statement.new('select  * from test where foo in (1, 2)  '), 'Database/test/select', 1.5
      ),
      NewRelic::Agent::SlowSql.new(
        NewRelic::Agent::Database::Statement.new("select * from test where foo in (1,2, 3 ,4,  5,6, 'snausage')"), 'Database/test/select', 1.2
      ),
      NewRelic::Agent::SlowSql.new(
        NewRelic::Agent::Database::Statement.new('select * from test2 where foo in (1,2)'), 'Database/test2/select', 1.1
      )
    ]
  ...
  def test_sql_id_fits_in_a_mysql_int_11
    ...
    assert(-2_147_483_648 <= sql_trace.sql_id, 'sql_id too small')
    assert(2_147_483_647 >= sql_trace.sql_id, 'sql_id too large')
  end
  ...
  def test_does_not_over_obfuscate_queries_for_postgres
    with_config('transaction_tracer.record_sql': 'obfuscated') do
      ...
      in_transaction do
        sql = %[INSERT INTO "items" ("name", "price") VALUES ('continuum transfunctioner', 100000) RETURNING "id"]
        sampler.notice_sql sql, 'Database/test/insert', { adapter: 'postgres' }, 1.23
      end
      sql_traces = sampler.harvest!
      assert_equal(%[INSERT INTO "items" ("name", "price") VALUES (?, ?) RETURNING "id"], sql_traces[0].sql)
    end
  end
  ...
  def test_takes_slowest_samples
    ...
    durations.each do |i|
      data.sql_data << NewRelic::Agent::SlowSql.new(
        NewRelic::Agent::Database::Statement.new("SELECT * FROM table#{i}"), "Database/table#{i}/select", i
      )
    end
  ...
  def test_to_collector_array
    with_config('transaction_tracer.explain_enabled': false) do
      ...
      expected = ['WebTransaction/Controller/c/a', '/c/a', 1_644_443_586,
  ...
  def test_to_collector_array_with_bad_values
    ...
    expected = ['path', 'uri', 1234, 'query', 'transaction',
                10, 12_000, 12_000, 12_000, params]
  ...
  def test_to_collector_array_with_database_instance_params
    statement = NewRelic::Agent::Database::Statement.new('query', nil, nil, nil, nil, 'jonan.gummy_planet', '1337',
                                                         'pizza_cube')
