...
module NewRelic::Agent
  class TransactionTest < Minitest::Test
    ...
    def test_notice_error_sends_uri_and_referer_from_request
      ...
      in_transaction(request: request) do |_txn|
        Transaction.notice_error('wat')
      end
    ...
    def make_transport_duration_timestamps(duration)
      ...
      [parent_timestamp, transaction_start]
    end
      ...
      with_config(rules: { ignore_url_regexes: [rule] }) do
        in_transaction do |txn|
          ...
          assert txn.user_defined_rules_ignore?,
                 "Paths should be ignored based on user defined rules. Rule: '#{rule}', Path: '#{txn.request_path}'."
        end
      end
    end
    ...
    def test_set_transaction_name_for_nested_transactions
      in_web_transaction 'Controller/Framework/webby' do |_t|
