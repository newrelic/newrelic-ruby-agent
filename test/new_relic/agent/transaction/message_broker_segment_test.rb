    ...
    class Transaction
      class MessageBrokerSegmentTest < Minitest::Test
        ...
        def test_segment_adds_cat_headers_to_message_properties_for_produce
          with_config "cross_application_tracer.enabled": true, "distributed_tracing.enabled": false,
                      cross_process_id: '321#123', encoding_key: 'abc' do
            in_transaction 'test_txn' do
              segment = NewRelic::Agent::Tracer.start_message_broker_segment(
                action: :produce,
                library: 'RabbitMQ',
              ...
              assert segment.headers.key?('NewRelicTransaction'),
                     'Expected message_properties to contain: NewRelicTransaction'
        ...
        def test_segment_adds_synthetics_and_cat_headers_to_message_properties_for_produce
          with_config "cross_application_tracer.enabled": true, "distributed_tracing.enabled": false,
            ...
            in_transaction 'test_txn' do |txn|
              ...
              assert segment.headers.key?('NewRelicTransaction'),
                     'Expected message_properties to contain: NewRelicTransaction'
              assert segment.headers.key?('NewRelicSynthetics'),
                     'Expected message_properties to contain: NewRelicSynthetics'
            end
          end
        end
        ...
        def test_segment_adds_distributed_trace_headers_to_message_properties_for_produce
          ...
          with_config "distributed_tracing.enabled": true,
            ...
            transaction = in_transaction 'test_txn' do |_txn|
              segment = NewRelic::Agent::Tracer.start_message_broker_segment(
                action: :produce,
                library: 'RabbitMQ',
                destination_type: :exchange,
