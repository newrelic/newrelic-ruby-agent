...
class AutostartTest < Minitest::Test
  ...
  def test_agent_wont_start_if_dollar_0_is_irb
    ...
    $0 = '/foo/bar/irb'
    assert !::NewRelic::Agent::Autostart.agent_should_start?,
           "Agent shouldn't autostart when process is invoked by irb"
  ensure
    $0 = @orig_dollar_0
  end
  ...
  RAILS_DEFAULT_RAKE_TASKS = %w[ about assets:clean assets:clobber
    ...
    define_method("test_agent_wont_autostart_if_top_level_rake_task_is_#{task}") do
      ...
      assert !::NewRelic::Agent::Autostart.agent_should_start?,
             "Agent shouldn't start during #{task.inspect} rake task"
    end
  end
  ...
  def test_denylisted_constants_can_be_configured
    with_config('autostart.denylisted_constants' => 'IRB,::AutostartTest::MyConst') do
      assert !::NewRelic::Agent::Autostart.agent_should_start?,
             "Agent shouldn't autostart when environment contains denylisted constant"
    end
  end
  ...
  def test_denylisted_executable_can_be_configured
    ...
    with_config('autostart.denylisted_executables' => 'boo,baz') do
      assert !::NewRelic::Agent::Autostart.agent_should_start?,
             "Agent shouldn't autostart when process is invoked by denylisted executable"
    end
  ensure
    $0 = @orig_dollar_0
  end
