  ...
  class BrowserMonitoringTest < Minitest::Test
    ...
    def test_should_only_instrument_successful_html_requests
      ...
      assert !app.should_instrument?({}, 500, { 'Content-Type' => 'text/html' }),
             'Expected not to instrument 500 requests.'
      assert !app.should_instrument?({}, 200, { 'Content-Type' => 'text/xhtml' }),
             'Expected not to instrument requests with content type other than text/html.'
    end

    def test_should_not_instrument_when_content_disposition
      assert !app.should_instrument?({}, 200,
                                     { 'Content-Type' => 'text/html', 'Content-Disposition' => 'attachment; filename=test.html' })
    end

    def test_should_not_instrument_when_already_did
      assert !app.should_instrument?({ NewRelic::Rack::BrowserMonitoring::ALREADY_INSTRUMENTED_KEY => true }, 200,
    ...
    def headers_from_request(headers, content)
      ...
      _, headers, = browser_monitoring.call({})
      headers
    end
  end
else
