...
def compare_metrics(expected, actual)
  ...
  assert_equal(expected.to_a.sort, actual.to_a.sort,
               "extra: #{(actual - expected).to_a.inspect}; missing: #{(expected - actual).to_a.inspect}")
end
...
def assert_metrics_recorded_exclusive(expected, options = {})
  ...
  recorded_metrics = recorded_metrics.select { |m| m.name.match(options[:filter]) } if options[:filter]
  recorded_metrics.reject! { |m| m.name.match(options[:ignore_filter]) } if options[:ignore_filter]

  expected_metrics   = expected.keys.map { |s| metric_spec_from_specish(s) }

  unexpected_metrics = recorded_metrics - expected_metrics
  unexpected_metrics.reject! { |m| m.name =~ %r{GC/Transaction} }
...
def assert_metrics_not_recorded(not_expected)
  ...
  not_expected.each do |specish, _|
    ...
    found_but_not_expected << spec if NewRelic::Agent.instance.stats_engine.to_h[spec]
  end
  assert_equal([], found_but_not_expected,
               "Found unexpected metrics: #{format_metric_spec_list(found_but_not_expected)}")
end
...
alias refute assert_false unless defined? refute
...
def in_transaction(*args)
  ...
  name = if opts.key?(:transaction_name)
           opts.delete(:transaction_name)
         else
           args.first || 'dummy'
         end
...
def in_web_transaction(name = 'dummy', &block)
  in_transaction(name, category: :controller, request: stub(path: '/'), &block)
end

def in_background_transaction(name = 'silly', &block)
  in_transaction(name, category: :task, &block)
end
...
def last_transaction_trace_request_params
  ...
  agent_attributes.each_with_object({}) do |(key, value), memo|
...
def find_last_transaction_node(transaction_sample = nil)
  root_node = if transaction_sample
                transaction_sample.root_node
              else
                last_transaction_trace.root_node
              end
...
def find_node_with_name(transaction_sample, name)
  transaction_sample.root_node.each_node do |node|
    return node if node.metric_name == name
  end
...
def find_node_with_name_matching(transaction_sample, regex)
  transaction_sample.root_node.each_node do |node|
    return node if node.metric_name.match regex
  end
...
def find_all_nodes_with_name_matching(transaction_sample, regexes)
  ...
  transaction_sample.root_node.each_node do |node|
    regexes.each do |regex|
      matching_nodes << node if node.metric_name.match regex
    end
  end
...
def with_config_low_priority(config_hash, &block)
  with_config(config_hash, false, &block)
end
...
unless Process.respond_to?(:__original_clock_gettime)
  Process.instance_eval do
    class << self
      ...
      alias __original_clock_gettime clock_gettime
...
def constant_path(name, opts = {})
  ...
  parts.each do |part|
    unless path.last.constants.include?(part.to_sym)
...
def wait_for_backtrace_service_poll(opts = {})
  ...
  until worker_loop.iterations > opts[:iterations]
    sleep(0.01)
    next unless Process.clock_gettime(Process::CLOCK_REALTIME) > deadline
    ...
    raise "Timed out waiting #{opts[:timeout]} s for backtrace service poll\n" +
          ...
          Thread.list.map do |t|
            "#{t}: newrelic_label: #{t[:newrelic_label].inspect}\n\n" +
              (t.backtrace || []).join("\n\t")
          end.join("\n\n")
  end
end
...
class EnvUpdater
  ...
  def self.inject(env)
    old_env = {}
    env.each { |key, _val| old_env[key] = ENV[key] }
    begin
      safe_update(env)
      yield
    ensure
...
def with_ignore_error_filter(filter)
...
def swap_instance_method(target, method_name, new_method_implementation)
...
def load_cross_agent_test(name)
  ...
  data = ::JSON.load(data)
  data.each { |testcase| testcase['testname'].gsub! ' ', '_' if testcase['testname'].is_a?(String) }
  data
end

def each_cross_agent_test(options, &block)
  options = { dir: nil, pattern: '*' }.update(options)
  path = File.join [cross_agent_tests_dir, options[:dir], options[:pattern]].compact
  Dir.glob(path).each(&block)
end
...
def assert_event_attributes(event, test_name, expected_attributes, non_expected_attributes)
  ...
  non_expected_attributes.each do |name|
    refute event_attrs[name],
           "Found value '#{event_attrs[name]}' for attribute '#{name}', but expected nothing in #{test_name}"
  end
end
...
def message_for_status_code(code)
  ...
  return Net::HTTP::STATUS_CODES[code] if defined?(Net::HTTP::STATUS_CODES)
...
def refute_transaction_noticed_error(txn, _error_class)
  error_segment = txn.segments.reverse.detect { |s| s.noticed_error }
  assert error_segment, 'Expected at least one segment with a noticed_error'
