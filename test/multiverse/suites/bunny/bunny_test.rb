...
class BunnyTest < Minitest::Test
  ...
  def test_cat_headers_not_read_for_pop_by_default
    ...
    with_queue do |queue|
      with_config "cross_application_tracer.enabled": true, cross_process_id: cross_process_id,
                  encoding_key: 'abc' do
        ...
        refute event[0].has_key?('nr.referringTransactionGuid'),
               "Event should not have key 'nr.referringTransactionGuid'"
  ...
  def test_segment_parameters_recorded_for_produce
    with_config('distributed_tracing.enabled': false) do
      ...
      node = find_node_with_name_matching last_transaction_trace, %r{^MessageBroker/}
  ...
  def test_pop_returns_original_message
    with_queue do |queue|
      ...
      assert msg.is_a?(Array), 'message was not an array'
      assert msg[0].is_a?(Bunny::GetResponse)
      assert msg[1].is_a?(Bunny::MessageProperties)
  ...
  def test_transaction_implicitly_created_for_consume
    ...
    with_config 'attributes.include': ['message.exchangeType'] do
      with_queue do |queue|
        ...
        queue.subscribe(block: false) do |_delivery_info, _properties, payload|
          lock.synchronize do
            msg = payload
            cond.signal
          end
  ...
  def test_noticed_error_at_segment_and_txn_on_error
    ...
    assert_segment_noticed_error txn, %r{^MessageBroker/RabbitMQ}, 'Timeout::Error', /timeout/i
  ...
  def with_queue(temp = true, exclusive = true)
