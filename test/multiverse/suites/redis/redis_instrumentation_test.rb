...
if NewRelic::Agent::Datastores::Redis.is_supported_version?
  class NewRelic::Agent::Instrumentation::RedisInstrumentationTest < Minitest::Test
    ...
    def test_noticed_error_at_segment_and_txn_on_error
      ...
      assert_segment_noticed_error txn, %r{Redis/connect$}, simulated_error_class.name, /error connecting/i
    ...
    def test_instrumentation_returns_expected_values
      ...
      assert_equal %w[OK OK], @redis.multi {
                                @redis.set('foo', 'bar')
                                @redis.set('baz', 'bat')
                              }
      assert_equal %w[bar bat], @redis.multi {
                                  @redis.get('foo')
                                  @redis.get('baz')
                                }
      assert_equal 2, @redis.del('foo', 'baz')

      assert_equal %w[OK OK], @redis.pipelined {
                                @redis.set('foo', 'bar')
                                @redis.set('baz', 'bat')
                              }
      assert_equal %w[bar bat], @redis.pipelined {
                                  @redis.get('foo')
                                  @redis.get('baz')
                                }
      assert_equal 2, @redis.del('foo', 'baz')
