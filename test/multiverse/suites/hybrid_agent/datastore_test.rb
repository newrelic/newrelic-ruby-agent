...
module NewRelic
  module Agent
    class DatastoreTest < Minitest::Test
      def setup
        @datastore_translator = NewRelic::Agent::OpenTelemetry::DatastoreTranslator
      end

      def test_parse_operation_returns_db_operation_name_when_present
        attributes = {'db.operation.name' => 'SELECT'}
        result = @datastore_translator.parse_operation('some_name', attributes)
      ...
      def test_parse_operation_prefers_db_operation_over_name
        ...
        result = @datastore_translator.parse_operation('SELECT', attributes)
      ...
      def test_parse_operation_returns_downcased_name_for_insert
        attributes = {}
        result = @datastore_translator.parse_operation('INSERT', attributes)
      ...
      def test_parse_operation_handles_mixed_case_known_operation
        attributes = {}
        result = @datastore_translator.parse_operation('CrEaTe', attributes)
      ...
      def test_parse_operation_parses_from_db_query_text_for_unknown_name
        ...
        result = @datastore_translator.parse_operation('unknown', attributes)
      ...
      def test_parse_operation_parses_from_db_statement_when_name_has_more_than_operation
        attributes = {'db.statement' => 'SELECT * FROM users WHERE id = 1'}
        result = @datastore_translator.parse_operation('INSERT users', attributes)
