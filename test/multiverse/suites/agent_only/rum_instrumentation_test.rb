...
class RumAutoTest < Minitest::Test
  ...
  def test_autoinstrumentation_with_basic_page_puts_header_at_beginning_of_head
    ...
    get '/'
    assert_response_includes(%(<html><head>#{CONFIG_REGEX}#{LOADER_REGEX}<title>foo</title></head>))
  end
  ...
  def test_autoinstrumentation_with_body_only_puts_header_before_body
    ...
    get '/'
    assert_response_includes %(<html>#{CONFIG_REGEX}#{LOADER_REGEX}<body>)
  end
  ...
  def test_autoinstrumentation_with_X_UA_Compatible_puts_header_after_meta_tag
    ...
    get '/'
    assert_response_includes(%(<html><head><meta http-equiv="X-UA-Compatible"/>#{CONFIG_REGEX}#{LOADER_REGEX}</head><body>))
  end
