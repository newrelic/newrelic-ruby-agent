...
if NewRelic::Agent::Instrumentation::Typhoeus.is_supported_version?
  ...
  class TyphoeusTest < Minitest::Test
    ...
    def test_maintains_on_complete_callback_ordering
      ...
      req.on_complete { |_rsp| invocations << :first }
      req.on_complete { |_rsp| invocations << :second }
      req.run

      assert_equal(%i[first second], invocations)
    end
    ...
    def test_tracing_succeeds_if_user_set_on_complete_callback_raises
      ...
      in_transaction('test') do
        req = Typhoeus::Request.new(default_url, ssl_option)
        req.on_complete { |_rsp| raise 'noodle' }
    ...
    def test_hydra
      ...
      trace.each_node do |node|
        ...
        assert node.params.key?(:exclusive_duration_millis),
               'Expected all nodes (except ROOT) to have :exclusive_duration_millis'
      end
