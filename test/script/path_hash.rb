...
def usage
  warn "Usage: path_hash.rb '<app_name>;<txn_name>' ['<app_name>;<txn_name>'...]"
  exit
end
...
path.each do |hop|
  ...
  if !app_name || !transaction_name
    warn ''
    warn "Error: Malformed transaction identifier '#{hop}'."
    usage
  end

  identifier = "#{app_name};#{transaction_name}"
  md5sum = Digest::MD5.digest(identifier)
  low4_of_md5 = md5sum.unpack1('@12N')
  ...
  puts "E: Rotated E:           0x#{rotated_path_hash.to_s(16).rjust(8,
                                                                     '0')} (0b#{rotated_path_hash.to_s(2).rjust(32,
                                                                                                                '0')})"
  puts ''
