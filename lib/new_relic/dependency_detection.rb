...
module DependencyDetection
  ...
  def detect!
    @items.each do |item|
      item.execute if item.dependencies_satisfied?
    end
  end
  ...
  class Dependent
    attr_reader :executed, :dependencies, :prepend_conflicts
    attr_accessor :name
    ...
    def execute
      @executes.each do |x|
        ...
        NewRelic::Agent.logger.error "Error while installing #{name} instrumentation:", e
        break
      end
    ensure
      executed!
    ...
    def check_dependencies
      ...
      dependencies.all? do |dep|
        ...
        NewRelic::Agent.logger.error("Error while detecting #{name}:", e)
        false
      end
    end
    ...
    def deprecated_disabled_configured?
      ...
      key = "disable_#{name}".to_sym
      return false unless ::NewRelic::Agent.config[key] == true

      ::NewRelic::Agent.logger.debug("Not installing #{name} instrumentation because of configuration #{key}")
      ::NewRelic::Agent.logger.debug \
        "[DEPRECATED] configuration #{key} for #{name} will be removed in the next major release." \
    ...
    def config_key
      return nil if config_name.nil?

      @config_key ||= "instrumentation.#{config_name}".to_sym
    end

    VALID_CONFIG_VALUES = %i[auto disabled prepend chain]
    ...
    def fetch_config_value(key)
      ...
      ::NewRelic::Agent.logger.debug("Using #{valid_value} configuration value for #{name} to configure instrumentation")
      valid_value
    end
    ...
    def prepend_conflicts?
      @prepend_conflicts.any? do |conflict|
        ...
        NewRelic::Agent.logger.error("Error while checking prepend conflicts #{name}:", e)
