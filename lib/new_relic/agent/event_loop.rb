    ...
    class EventLoop
      class Timer
        ...
        def calculate_next_fire_time
          ...
          fire_time += @interval while fire_time <= now
          fire_time
        end
      ...
      def wait_to_run(nonblock)
        ...
        @self_pipe_rd.read(1) if ready && ready[0] && ready[0][0] && ready[0][0] == @self_pipe_rd
      end

      def fire_timers
        @timers.each do |_event, timer|
          fire_timer(timer)
        end
      end
      ...
      def prune_timers
        @timers.delete_if { |_e, t| t.finished? }
      end
      ...
      def dispatch_event(event, args)
        ...
        unless errors.empty?
          ::NewRelic::Agent.logger.error "#{errors.size} error(s) running task for event '#{event}' in Agent Event Loop:",
                                         *errors
        end
      end
