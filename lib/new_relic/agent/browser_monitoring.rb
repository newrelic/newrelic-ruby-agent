...
<<-eos
<script src=#{file} type="text/javascript"></script><script type="text/javascript" charset="utf-8">NR_EUM.setContext("#{beacon}","#{license_key}","#{application_id}","#{transaction_name}")</script>
eos
      end
...
<<-eos
<script type="text/javascript" charset="utf-8">NR_EUM.recordFooter(#{queue_time},#{app_time})</script>
eos
        end
      end
    end
