  ...
  module Agent
    class AgentLogger
      ...
      def log_exception(level, e, backtrace_level = level)
        @log.send(level, format('%p: %s', e.class, e.message))
      ...
      def find_or_create_file_path(path_setting, root)
        [File.expand_path(path_setting),
         File.expand_path(File.join(root, path_setting))].each do |abs_path|
          return abs_path[%r{^(.*?)/?$}] if File.directory?(abs_path) || begin
            Dir.mkdir(abs_path)
