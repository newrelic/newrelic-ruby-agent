      ...
      class TraceContext
        ...
        class << self
          ...
          def format_trace_parent(trace_id: nil,
                                  parent_id: nil,
                                  trace_flags: nil)
            format TRACE_PARENT_FORMAT_STRING,
                   VERSION,
                   trace_id,
                   parent_id,
                   trace_flags
          ...
          def extract_traceparent(format, carrier)
            ...
            if matchdata = header.match(TRACE_PARENT_REGEX)
              TRACE_PARENT_REGEX.named_captures.each_with_object({}) do |(name, (index)), hash|
          ...
          def decode_payload(payload)
            ...
            NewRelic::Agent.increment_metric SUPPORTABILITY_TRACE_STATE_INVALID_NR_ENTRY if payload
            nil
          end
        end
