      ...
      module Trace
        class Tracer # < ::OpenTelemetry::Trace::Tracer
          ...
          def start_root_span(name, attributes: nil, links: nil, start_timestamp: nil, kind: nil)
            # NewRelic::Agent::Tracer.start_transaction may be the equivalent method
          end
            ...
            when :internal
              begin
                ...
                if finishable.is_a?(NewRelic::Agent::Transaction::Segment)
                  span = FakeSpan.new(segment: finishable, transaction: finishable.transaction)
                else
                  # this happens, not sure what test case
                  # also happens in the "outer" example for simple.rb
                  span = FakeSpan.new(segment: finishable.segments.first, transaction: finishable)
                end
          ...
          def find_context
            if ::OpenTelemetry::Context.current == ::OpenTelemetry::Trace::Span::INVALID
