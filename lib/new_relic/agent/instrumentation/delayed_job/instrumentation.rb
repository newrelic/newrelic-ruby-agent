    ...
    module Instrumentation
      module DelayedJob
        def initialize_with_tracing
          yield
          worker_name = if respond_to?(:name) then name
                        elsif self.class.respond_to?(:default_name) then self.class.default_name
                        end
