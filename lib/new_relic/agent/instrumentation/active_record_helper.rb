    ...
    module Instrumentation
      module ActiveRecordHelper
        ...
        def instrument_relation_methods
          ::ActiveRecord::Relation.class_eval do
            ...
            def update_all(*args, &blk)
              ::NewRelic::Agent.with_database_metric_name(name, nil, ACTIVE_RECORD) do
        ...
        def product_operation_collection_for(name, sql, adapter_name)
          ...
          NewRelic::Agent::Datastores::MetricHelper.product_operation_collection_for product, operation, model,
                                                                                     ACTIVE_RECORD
        end
        ...
        def model_from_splits(splits)
          splits.first if splits.length == 2
        end
        ...
        module InstanceIdentification
          ...
          SUPPORTED_ADAPTERS = %i[mysql postgres].freeze
          ...
          def mysql_default_case?(config, adapter)
            %i[mysql2 mysql].include?(adapter) &&
