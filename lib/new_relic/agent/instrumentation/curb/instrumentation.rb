      ...
      module Curb
        ...
        module Multi
          ...
          def add_with_tracing(curl)
            if curl.respond_to?(:_nr_serial) && curl._nr_serial && NewRelic::Agent::Tracer.tracing_enabled?
              hook_pending_request(curl)
            end
          ...
          def perform_with_tracing(&block)
            return yield if first_request_is_serial?

            trace_execution_scoped('External/Multiple/Curb::Multi/perform', &block)
          end
          ...
          def hook_pending_request(request)
            ...
            NewRelic::Agent.logger.error('Untrapped exception', e)
          end
          ...
          def wrap_request(request)
            [NewRelic::Agent::HTTPClients::CurbRequest.new(request),
          ...
          def first_request_is_serial?
            return false unless (first = requests.first)
