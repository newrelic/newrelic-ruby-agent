    ...
    module MethodTracer
      ...
      def self.included target
        ...
        NewRelic::Agent.logger.warn \
          "`include NewRelic::Agent::MethodTracer` is deprecated\n" \
          "Please use `prepend NewRelic::Agent::MethodTracer` instead"
      end
      ...
      module PrependMethods
        ...
        module PrependMethodTracer
          ...
          def method_without_push_scope(method_name, metric_name_code, options)
            define_method _traced_method_name(method_name, metric_name_code) do |*arguments|
              eval assemble_code_header(method_name, metric_name_code, options)
              t0 = Time.now
              begin
                super(*arguments)
              ensure
          ...
          def method_with_push_scope(method_name, metric_name_code, options)
            define_method _traced_method_name(method_name, metric_name_code) do |*arguments|
              eval options[:code_header]
              result = ::NewRelic::Agent::MethodTracerHelpers.trace_execution_scoped(metric_name_code, :metric => options[:metric]) do
                super(*arguments)
              end
              eval options[:code_footer]
              result
          ...
          def define_traced_method(method_name, metric_name_code, options)
        ...
        def _add_method_tracer_now(method_name, metric_name_code, options)
          ...
          define_traced_method(method_name, metric_name_code, options)

          ::NewRelic::Agent.logger.debug("Traced method with Prepend: class = #{derived_class_name},"+
                                         "method = #{method_name}, "+
