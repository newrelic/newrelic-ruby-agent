...
module NewRelic::Agent
  ...
  private
    ...
    def do_close
      return if closed
#      NewRelic::Agent.instance.send_traced_session(self)
      @history = []
      @closed = true
    end
  ...
  private
    def trace_request
      ...
      if params[:capture_session] == 'start'
        session[:session_tracer] = NewRelic::Agent::SessionTracer.new
...
module ActiveRecord
  class Base
    def create_or_update_with_notice_write
      NewRelic::Agent::SessionTracer.notice_database_write(self.class.name)
