  ...
  module NewRelicInstrumentation
    ...
    def notice_sql(sql)
      ...
      if current_segment.sql_statement
        ...
        current_segment._notice_sql sql, opts, explainer_for(sql)
      end
    end
    ...
    def explainer_for(sql)
      proc do |*|
        if THREAD_SAFE_CONNECTION_POOL_CLASSES.include?(pool.class)
          self[sql].explain
        else
          NewRelic::Agent.logger.log_once(:info, :sequel_explain_skipped,
                                          'Not running SQL explains because Sequel is not in recognized multi-threaded mode')
          nil
        end
      end
    end
