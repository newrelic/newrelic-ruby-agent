...
guard :minitest, guard_options do
  ...
  watch(%r{^test/fixtures/cross_agent_tests/distributed_tracing/(.+).json}) do |m|
    "test/new_relic/agent/distributed_tracing/#{m[1]}_cross_agent_test.rb"
  end
  watch('test/test_helper.rb')  { 'test/new_relic' }
  watch('test/agent_helper.rb') { 'test/new_relic' }
  watch('lib/new_relic/agent/configuration/default_source.rb') do
    'test/new_relic/agent/configuration/orphan_configuration_test.rb'
  end
  watch(%r{^lib/new_relic/agent/transaction/(.+).rb}) do |m|
    "test/new_relic/agent/distributed_tracing/#{m[1]}_cross_agent_test.rb"
  end
end
