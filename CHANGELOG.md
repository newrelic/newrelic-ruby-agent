...
- **Bugfix: Enforce a maximum size for Custom Event attributes**
  ...
  Previously, the agent would allow custom event attributes to be any size. This would lead to the New Relic backend dropping the attribute that was larger than the maximum size. Now, the agent will truncate custom even attributes to limit the size to 4095 bytes. [PR#2401](https://github.com/newrelic/newrelic-ruby-agent/pull/2401)
