...
Version <dev> of the agent adds the ability to filter logs by level, an API to add custom attributes to logs, expands instrumentation for Action Cable, and provides a bugfix for Code-Level Metrics.
...
- **Feature: Custom attributes for logs API**
  ...
  For example: `NewRelic::Agent.add_custom_log_attributes(dyno: ENV.fetch('DYNO'), pod_name: ENV.fetch('POD_NAME'))`, will add the attributes `dyno` and `pod_name` to your log events. Attributes passed to this API will be added to all log events.
